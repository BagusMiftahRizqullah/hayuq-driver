...
const DrivingLicenseForm = ({setActiveForm}) => {
  const componentStyles = styles();
  const dispatch = useDispatch();
  const {
    auth: {dataUser},
    registerProcessReducer: {isLoading, dataLicense},
  } = useSelector((state) => state);
  const [openModal, setOpenModel] = useState(false);
  const [modalCalendar, setShowModalCalendar] = useState(false);
  const [errorUpload, setErrorUpload] = useState(false);
  const [modalUpload, setModalUpload] = useState(false);
  const [imageList, setImageList] = useState([dataLicense?.path]);
  const [filledForm, setFilledForm] = useState(false);
  const [errorSubmit, setErrorSubmit] = useState(false);
  const [dataForm, setDataForm] = useState({
    year: dataLicense?.year || '',
    pictures: dataLicense?.path || '',
  });
  ...
  const handleSubmitDataLicense = () => {
    const idDriver = dataUser.id;
    dispatch({
      type: REGISTER_PROCESS.REGISTER_ADD_LICENSE,
      payload: {drivers_id: idDriver, licenses: dataForm},
      meta: {
        success: function(){
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <KeyboardAvoidingView style={[Layout.flex]}>
            ...
            <View style={componentStyles.cardDocument}>
              {imageList[0] === undefined ? (
                ...
                <VStack style={Layout.flexCenterMid}>
                  ...
                  />
                  <Text style={Fonts.textXsBold}>Driving License</Text>
                  <Text
                    style={{
                      ...Fonts.textXs,
          ...
          <VStack
            ...
            {filledForm | dataLicense?.document_status?.includes('rejected') ? (
              <Button
                style={{margin: CustomSpacing(16)}}
      ...
      <ModalBottom
        ...
        <View style={componentStyles.modalStyle}>
          ...
          <HStack style={[Layout.flexCenterMid]}>
            ...
            <Button
              onPress={!isLoading && handleSubmitDataLicense}
              style={{paddingHorizontal: CustomSpacing(50)}}>
