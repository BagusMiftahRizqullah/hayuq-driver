...
import {ArrowDownIcon, DummyImage, ErrorUploadIcon} from '@assets';
...
const StnkForm = ({setActiveForm}) => {
  const componentStyles = styles();
  const dispatch = useDispatch();
  const {
    auth: {dataUser},
    registerProcessReducer: {isLoading, dataVehicle},
    masterReducer: {dataListVehicle},
  } = useSelector((state) => state);
  const [openModal, setOpenModel] = useState(false);
  const [modalUpload, setModalUpload] = useState(false);
  const [modalCalendar, setShowModalCalendar] = useState(false);
  const [errorUpload, setErrorUpload] = useState(false);
  const [imageList, setImageList] = useState([dataVehicle?.path]);
  const [filledForm, setFilledForm] = useState(false);
  const [errorSubmit, setErrorSubmit] = useState(false);
  const [dataForm, setDataForm] = useState({
    vehicles_id: dataVehicle?.vehicles_id || '',
    model: dataVehicle?.model || '',
    plate_number: dataVehicle?.plate_number || '',
    year: dataVehicle?.year || '',
    licenses_expired_date: dataVehicle?.licenses_expired_date || '',
    pictures: dataVehicle?.path || '',
    licenses: dataVehicle?.licenses || '',
  });
  ...
  const renderExpiredDate = () => {
    if (dataForm.licenses_expired_date) {
      return moment(dataForm.licenses_expired_date).format('YYYY-MM-DD')
    } else {
  ...
  const handleSubmitStnk = () => {
    const idDriver = dataUser.id;
    dispatch({
      type: REGISTER_PROCESS.REGISTER_ADD_VEHICLE,
      payload: {drivers_id: idDriver, vehicles: dataForm},
      meta: {
        success: function(){
          setOpenModel(false);
          setActiveForm('');
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <KeyboardAvoidingView style={[Layout.flex]}>
          <VStack style={{marginHorizontal: CustomSpacing(16)}}>
            ...
            <Text style={{...Fonts.textXs, marginBottom: CustomSpacing(10)}}>
              Please complete the details of your motorcycle. Note that your
              motorcycle age must not exceeded 10 years and is not more than 250
              cc.
            </Text>
            ...
            <SelectDropdown
              ...
              defaultValueByIndex={dataForm.vehicles_id - 1}
              search={true}
            ...
            <TextInput
              ...
              onChangeText={(e) => setDataForm((prev) => ({...prev, model: e}))}
            />
            <Text style={{...Fonts.textSBold, marginBottom: CustomSpacing(10)}}>
              Vehicle Year
            </Text>
            <TextInput
              ...
              value={dataForm.year.toString()}
              onChangeText={(e) => setDataForm((prev) => ({...prev, year: e}))}
            />

            <Text style={{...Fonts.textSBold, marginBottom: CustomSpacing(10)}}>
              Vehicle Registration Certificate Detail
            </Text>
            <Text style={{...Fonts.textXs, marginBottom: CustomSpacing(10)}}>
              Please complete the details of your Vehicle Registration
              Certificate (STNK). Make sure you have a valid & updated STNK.
            </Text>
            ...
            <TextInput
              style={componentStyles.inputStyle}
              placeholder="e.g. B 2198 SUI"
              value={dataForm.plate_number}
              onChangeText={(e) =>
                setDataForm((prev) => ({...prev, plate_number: e}))
              }
            />
            <Text style={{...Fonts.textSBold, marginBottom: CustomSpacing(10)}}>
              License Number (STNK)
            ...
            <TextInput
              style={componentStyles.inputStyle}
              placeholder="e.g. 14008526"
              keyboardType="numeric"
              value={dataForm.licenses}
              onChangeText={(e) =>
                setDataForm((prev) => ({...prev, licenses: e}))
              }
            />
            <Text style={{...Fonts.textSBold, marginBottom: CustomSpacing(10)}}>
              License Expired
          ...
          <VStack
            ...
            {filledForm | dataVehicle?.document_status?.includes('rejected') ? (
              <Button
      ...
      <ModalBottom
        ...
        <View style={componentStyles.modalStyle}>
          <VStack bottom={CustomSpacing(56)}>
            <Text style={{...Fonts.textMBold}}>STNK Confirmation</Text>
            <Text
              style={{
                ...Fonts.textM,
          ...
          <HStack style={[Layout.flexCenterMid]}>
            ...
            <Button
              onPress={!isLoading && handleSubmitStnk}
              style={{paddingHorizontal: CustomSpacing(50)}}>
      ...
      <ModalBottom
        ...
        <View style={componentStyles.modalStyle}>
          <Calendar
            ...
            onDayPress={(day) => {
              setDataForm((prev) => ({...prev, licenses_expired_date: day.dateString}));
